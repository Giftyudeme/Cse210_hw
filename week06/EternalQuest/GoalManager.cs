...
public class GoalManager
{
    ...
    private void DisplayPlayerInfo()
    {
        ...
        if (level < 1) level = 1; // Prevent negative levels

        string rank = "Novice Adventurer";
        if (level >= 3) rank = "Scripture Warrior";
        if (level >= 5) rank = "Temple Vanguard";
        if (level >= 10) rank = "Level 13 Ninja Unicorn";

        Console.WriteLine($"=== Current Score: {_score} ===");
        Console.WriteLine($"=== Rank: {rank} (Level {level}) ===");
    }
    ...
    private void RecordEvent()
    {
        ...
        if (index >= 0 && index < _goals.Count)
        {
            ...
            {
                Console.WriteLine($"Ouch! You lost {Math.Abs(pointsEarned)} points for breaking a habit.");
            }
            else
            {
